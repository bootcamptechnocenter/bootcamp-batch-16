...
namespace IDMS.Web.Services.Impl
{
    public class MstModelsService : IMstModelsService
    {
        ...
        public MstModelsService(IHttpClientFactory httpClientFactory, IHttpContextAccessor httpContextAccessor)
        {
        ...
        public async Task<PagedResult<ResMstModelsDto>> GetAllModels(ReqBaseParamDto dto)
        {
            ...
            var url = $"api/models/models?page={dto.Page}&limit={dto.Limit}&search={Uri.EscapeDataString(dto.Search ?? string.Empty)}";
            var response = await client.GetAsync(url);
            response.EnsureSuccessStatusCode();

            var content = await response.Content.ReadAsStringAsync();
            var result = JsonSerializer.Deserialize<ApiClientResponse<List<ResMstModelsDto>>>(content, _jsonOptions);

            return new PagedResult<ResMstModelsDto>
            {
                Items = result?.Data ?? new List<ResMstModelsDto>(),
        ...
        {
            var client = CreateHttpClient();
            var response = await client.GetAsync($"api/models/model/{id}");
            if (!response.IsSuccessStatusCode) return null;

            var content = await response.Content.ReadAsStringAsync();
            var result = JsonSerializer.Deserialize<ApiClientResponse<ResMstModelsDto>>(content, _jsonOptions);
            return result?.Data;
        }
        ...
        public async Task<bool> CreateModel(ReqCreateModelDto dto)
        {
            ...
            var response = await client.PostAsync("api/models/model", content);
