...
{
    public class MasterModelClientService : IMasterModelService
    {
        ...
        public MasterModelClientService(IHttpClientFactory httpClientFactory, IHttpContextAccessor httpContextAccessor)
        {
        ...
        {
            var client = CreateClient();
            var response = await client.DeleteAsync($"master/general/model/{id}?deletedBy={Uri.EscapeDataString(deletedBy)}");
            return response.IsSuccessStatusCode;
        }
    }
}