    ...
    public class MstModelService : IMstModelService
    {
        ...
        public async Task<PagedResult<ResMstModelDto>> GetMstModel(ReqBaseParamDto dto)
        {
            var query = _context.MstModels
                .Include(x => x.Type)
                    .ThenInclude(t => t.Brand)  // gunakan ThenInclude, bukan Include terpisah
                .AsQueryable();
            ...
            var items = await query
                ...
                {
                    Id       = x.Id,
                    BrandId   = x.Type != null && x.Type.Brand != null ? x.Type.Brand.Id   : 0,
                    ...
                    Year     = x.Year,
                    IsActive = x.IsActive,
                    // hitung stock yang IsActive && IsReady (belum terjual)
                    StockCount = _context.MstStocks
                        .Count(s => s.ModelId == x.Id && s.IsReady && s.DeletedAt == null)
                })
                .ToListAsync();
