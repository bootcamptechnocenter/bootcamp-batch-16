...
namespace IDMS.Modules.Master.Services.Impl
{
    public class MstStockService : IMstStockService
    {
        ...
        {
            var isExist = await _context.MstStocks.AnyAsync(x => x.ModelId == dto.ModelId && x.DeletedAt == null);
            if (isExist) throw new ArgumentException("Stock for this Model already exists. Please update the existing stock.");
        ...
        public  async Task<PagedResult<ResMstStockDto>> GetMstStock(ReqBaseParamDto dto)
        {
            ...
            var items = await query
                ...
                .Select(x => new ResMstStockDto
                {
                    Id = x.Id,
                    ModelId = x.ModelId,
                    NamaMobil = (x.Model != null && x.Model.Type != null && x.Model.Type.Brand != null) 
                        ? $"{x.Model.Type.Brand.Name} {x.Model.Type.Name} {x.Model.Name}" 
                        : "Unknown",
        ...
        public async Task<bool> UpdateMstStock(ReqUpdateMstStockDto dto, int id)
        {
            ...
            var isExist = await _context.MstStocks.AnyAsync(x => x.ModelId == dto.ModelId && x.Id != id && x.DeletedAt == null);
            if (isExist) throw new ArgumentException("Another stock record already exists for this Model.");
